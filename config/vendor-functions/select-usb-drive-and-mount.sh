...
while true; do
    ...
    if [[ "${usb_drives_json}" == "[]" ]]; then
        read -r -p "No USB drives found. Press enter to try again. "
    else
        ...
        while true; do
            read -r -p "Enter the index (0, 1, etc.) of the USB drive that you'd like to use, or press enter to search for USB drives again: " index
            if [[ -z "${index}" ]]; then
