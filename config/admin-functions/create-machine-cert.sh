...
function get_machine_private_key_password_from_user_input() {
    local prompt="Enter a private key password (at least 4 characters long): "
    local validation_error_message="Expected private key password to be at least 4 characters long"
    local machine_private_key_password
    while true; do
        read -p "${prompt}" machine_private_key_password
        if [[ "${#machine_private_key_password}" -ge 4 ]]; then
    ...
    echo "${machine_private_key_password}"
}
...
# new key
machine_private_key_password="$(get_machine_private_key_password_from_user_input)"
