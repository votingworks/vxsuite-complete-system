...
then
    ...
    sudo /vx/code/config/ui-functions/manage-usb-drive.sh --mount $DEVICE usb-drive
fi
...
# unmount the USB stick and make sure it's all written to disk
echo "Saving logs to USB drive..."
sync $MOUNTPOINT
sudo /vx/code/config/ui-functions/manage-usb-drive.sh --unmount usb-drive
